...
class DocumentRequirementsTest < ActiveSupport::TestCase
  ...
  test '5.6.3 (O)' do
    ...
    assert_has_many :document_descriptions, :documents, as: :documentable
    assert_belongs_to :document, :documentable, polymorphic: true

    document_description = create(:document_description)

    document_a = create(:document, documentable: document_description)
    document_b = create(:document, documentable: document_description)

    assert document_description.documents.count == 2
    assert document_a.documentable == document_description
    assert document_b.documentable == document_description
  end
  ...
  test '5.6.4 (B)' do
    ...
    assert_has_many :record, :documents, as: :documentable
    assert_belongs_to :document, :documentable, polymorphic: true

    record = create(:record, :for_filing)

    document_a = create(:document, documentable: record)
    document_b = create(:document, documentable: record)

    assert record.documents.count == 2
    assert document_a.documentable == record
    assert document_b.documentable == record
  end
  ...
  test '5.6.6 (O)' do
    ...
    document_description = create(:document_description)
    create(:document, documentable: document_description, document_version: '1.0')
    create(:document, documentable: document_description, document_version: '1.1')

    assert document_description.documents.count == 2
    assert document_description.documents.map(&:document_version) == %w(1.1 1.0)
  end
  ...
  test '5.6.7 (B)' do
    ...
    document_description = create(:document_description)
    create(:document, documentable: document_description, document_variant: 'English')
    create(:document, documentable: document_description, document_variant: 'Norwegian')

    assert document_description.documents.count == 2
    assert document_description.documents.map(&:document_variant) == %w(Norwegian English)
  end
  ...
  test '5.6.8 (O)' do
    ...
    document_description = create(:document_description)
    create(:document, documentable: document_description, document_format: 'PDF')
    create(:document, documentable: document_description, document_format: 'DOCX')

    assert document_description.documents.count == 2
    assert document_description.documents.map(&:document_format) == %w(DOCX PDF)
  end
